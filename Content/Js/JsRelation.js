...
function loadList(json) {
    ...
    if (json.Data && json.Data.length > 0) {
        for (var i = 0; i < json.Data.length; i++) {
            ...
            html += '<td>' + (data.State > 0 ? '<a onclick="onCance(\'' + data.ID + '\')">取消发布</a>' : '<a onclick="onPublish(\'' + data.ID + '\')">发布</a>&nbsp;&nbsp;<a href="/Relation/Add?ID=' + data.ID + '" >修改</a>&nbsp;&nbsp;<a onclick="onDel(\'' + data.ID + '\')">删除</a>') + '</td>';
            html += '</tr>';
        }
    }
    $('#tbList').html(html);
...
function onPublish(id) {
    if (confirm("是否确认发布？")) {
        var ajax = {};
        ajax.type = "POST";
        ajax.url = "/Relation/NoticePublish";
        ajax.data = {};
        ajax.data.ID = id;
        ajax.data.ajax = new Date().getTime();
        ajax.success = function (result) {
